        ...
        <div className="space-y-8">
          <div className="space-y-3">
            ...
            <div className="relative group">
              <input type="number" value={emi.amount} onChange={(e) => updateEmi({ amount: Number(e.target.value) })} className="horizon-input bg-slate-700/50 text-white border-white/10 group-hover:border-primary/50 transition-all" />
          ...
          <div className="space-y-3">
            ...
            <div className="relative group">
              <input type="number" value={emi.rate} onChange={(e) => updateEmi({ rate: Number(e.target.value) })} className="horizon-input bg-slate-700/50 text-white border-white/10 group-hover:border-primary/50 transition-all" />
              <div className="absolute right-5 top-1/2 -translate-y-1/2 text-white/30 font-black text-lg">%</div>
            </div>
          </div>
          <div className="space-y-3">
            ...
            <div className="relative group">
              <input type="number" value={emi.years} onChange={(e) => updateEmi({ years: Number(e.target.value) })} className="horizon-input bg-slate-700/50 text-white border-white/10 group-hover:border-primary/50 transition-all" />
      ...
      results={
        <>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="horizon-card p-8 flex items-center gap-6 group">
            ...
            <div>
              <h3 className="text-[10px] font-black text-white uppercase tracking-widest mb-1.5">Total Repayment</h3>
              <p className="text-3xl font-black text-black tracking-tight">₹{results.totalRepayment.toLocaleString('en-IN')}</p>
            </div>
          </motion.div>
        </>
      }
