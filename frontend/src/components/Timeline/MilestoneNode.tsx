  ...
  return (
    ...
    >
      <div className="flex flex-col items-center -translate-y-full">
        <div
          className={`p-3 bg-white border ${isOverlay ? 'border-indigo-500 shadow-2xl scale-105' : 'border-slate-200 shadow-lg shadow-slate-200/50'} rounded-2xl text-xs transition-all duration-75 w-40 text-center group`}
        >
          ...
          <strong className="block font-bold truncate text-slate-900 text-sm tracking-tight">{milestone.label}</strong>
          <div className="flex items-center justify-center gap-2 mt-1.5">
            <div className="flex flex-col">
              <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Age</span>
              <span className="text-slate-900 font-extrabold">{milestone.age}</span>
            </div>
        ...
        {/* Anchor Line & Dot */}
        <div className={`w-1 h-10 bg-slate-200 mt-1 transition-opacity ${isOverlay ? 'opacity-50' : 'opacity-100'}`}></div>
